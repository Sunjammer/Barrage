...
import barrage.data.BarrageItemDef;
...
class Parser {
	...
	static private function buildActionDef(b:Block, anon:Bool = false):ActionDef {
		...
		var ad:ActionDef;
		if (anon)
			ad = new ActionDef(uniqueName());
		else
			ad = new ActionDef(b.values[1]);
	...
	static function buildBulletDef(b:Block, anon:Bool = false):BulletDef {
		var bd:BulletDef;
		if (anon)
			bd = new BulletDef(uniqueName());
		else
			bd = new BulletDef(b.values[1]);
	...
	static function getBlock(start:Int, lines:Array<Line>, out:Array<Block>):Int {
		...
		while (true) {
			...
			if (lines[count].level <= root.level)
				break;
	...
	static function readStatement(b:Block):Void {
		...
		switch (b.tokens) {
			case [
				...
				TNumber | TConst_math | TScript
			]:
				runPropertyInit(b);
			case [
				TSet | TIncrement,
				TSpeed | TDirection | TAcceleration,
				TNumber | TConst_math | TScript | TAimed
			]:
			...
			case [TWait, TNumber | TConst_math | TScript, TFrames | TSeconds]:
				runWait(b);
			case [TRepeat, TNumber | TConst_math | TScript | TForever]:
				runRepeat(b);
			case [TVanish]:
				runVanish(b);
			default:
	...
	static private function runFireParse(b:Block) {
		...
		while (index < b.tokens.length) {
			switch (b.tokens.slice(index, index + 4)) {
				case [
					TAt,
					TIncremental | TAbsolute | TRelative,
					TSpeed,
					TNumber | TConst_math | TScript
				]:
				...
				case [
					TIn,
					TIncremental | TAbsolute | TRelative | TAimed,
					TDirection,
					TNumber | TConst_math | TScript
				]:
